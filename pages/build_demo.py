...
(function(){
  ...
  function fakeDb(names){
    ...
    names.forEach(function(name){
      var dbid=loadSchema()[name].dbid;
      db.query=function(){return Promise.resolve({results:loadLS(name),hasMore:false,nextCursor:null});};
      db.addRecord=function(opts){
        var rows=loadLS(name);
        var id='rec_'+Date.now()+'_'+Math.floor(Math.random()*1e6);
        var rec={_id:id,record_id:id,id:id};
        Object.keys(opts.properties||{}).forEach(function(k){rec[k]=opts.properties[k]});
        rows.push(rec);saveLS(name,rows);
        return Promise.resolve({_id:id,record_id:id});
      };
      db.updateRecord=function(opts){
        var rows=loadLS(name);
        var i=rows.findIndex(function(r){return (r._id||r.record_id)===opts.recordId;});
        if(i>=0){Object.keys(opts.properties||{}).forEach(function(k){rows[i][k]=opts.properties[k]});saveLS(name,rows);}
        return Promise.resolve({ok:true});
      };
      db.deleteRecord=function(opts){
        var rows=loadLS(name);
        saveLS(name,rows.filter(function(r){return (r._id||r.record_id)!==opts.recordId;}));
        return Promise.resolve({ok:true});
      };
      db.getSchema=function(){
        var sch={
          JOBS_ID_FAKE:{properties:[]},
          APPS_ID_FAKE:{properties:[]},
          INTERN_ID_FAKE:{properties:[]}
        }[dbid];
  ...
  if(!hasSDK()){
    ...
    document.addEventListener('DOMContentLoaded',function(){
      ...
      b.style.cssText='position:fixed;top:0;left:0;right:0;z-index:99;background:#fef3c7;color:#92400e;text-align:center;padding:8px;font-size:13px;border-bottom:1px solid #fbbf24';
      b.innerHTML='🌐 GitHub Pages 演示模式：数据存在浏览器 localStorage。<a href="javascript:void(0)" onclick="if(confirm(\'清空演示数据？\')){localStorage.clear();location.reload();}" style="color:#b45309;text-decoration:underline">清空演示数据</a>';
    ...
    if(!localStorage.getItem('qiuzhao_demo_seeded_v1')){
      var demoJobs=[
        {公司:{text:'字节跳动'},批次:{text:'27秋招'},岗位方向:{text:'大模型算法、推荐算法'},工作地点:{text:'北京、上海'},优先级:{text:'P0'},投递状态:{text:'待投递'},网申开始:{date:'2026-09-01'},截止日期:{date:'2026-09-30'},投递链接:{url:{text:'官网',link:'https://jobs.bytedance.com'}},来源:{text:'牛客校招日程'},备注:{text:'示例数据，演示用'}},
        {公司:{text:'中国电信'},批次:{text:'27秋招'},岗位方向:{text:'云计算、AI'},工作地点:{text:'北京'},优先级:{text:'P1'},投递状态:{text:'待投递'},截止日期:{date:'2026-10-20'},投递链接:{url:{text:'网申',link:'https://zhaopin.chinatelecom.com.cn'}},来源:{text:'牛客校招日程'},备注:{text:'央国企示例'}},
        {公司:{text:'中国移动'},批次:{text:'27秋招'},岗位方向:{text:'人工智能、数据'},工作地点:{text:'成都'},优先级:{text:'P1'},投递状态:{text:'待投递'},截止日期:{date:'2026-11-10'},投递链接:{url:{text:'网申',link:'https://job.10086.cn'}},来源:{text:'牛客校招日程'},备注:{text:'央国企示例'}},
        {公司:{text:'腾讯'},批次:{text:'27暑期实习'},岗位方向:{text:'机器学习'},工作地点:{text:'深圳、北京'},优先级:{text:'P0'},投递状态:{text:'待投递'},截止日期:{date:'2026-10-05'},投递链接:{url:{text:'官网',link:'https://careers.tencent.com'}},来源:{text:'牛客校招日程'},备注:{text:'互联网示例'}},
        {公司:{text:'美团'},批次:{text:'27秋招'},岗位方向:{text:'后端、算法'},工作地点:{text:'北京'},优先级:{text:'P2'},投递状态:{text:'待投递'},截止日期:{date:'2026-10-12'},投递链接:{url:{text:'官网',link:'https://zhaopin.meituan.com'}},来源:{text:'牛客校招日程'},备注:{text:'互联网示例'}},
        {公司:{text:'商汤科技'},批次:{text:'27秋招'},岗位方向:{text:'AGI算法、视觉算法'},工作地点:{text:'北京、上海'},优先级:{text:'P1'},投递状态:{text:'待投递'},截止日期:{date:'2026-10-08'},投递链接:{url:{text:'官网',link:'https://www.sensetime.com/cn/careers'}},来源:{text:'牛客校招日程'},备注:{text:'互联网示例'}}
      ];
      var demoApps=[{公司:{text:'字节跳动'},岗位:{text:'大模型算法实习生'},当前阶段:{text:'一面'},投递日期:{date:'2026-09-08'},下次节点:{date:'2026-09-15'},节点说明:{text:'技术一面'},复盘笔记:{text:'演示数据'},相关链接:{url:{text:'官网',link:'https://jobs.bytedance.com'}}}];
