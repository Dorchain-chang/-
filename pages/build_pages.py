...
:root{
  ...
  --pri-rgb:59,91,253; --pri2-rgb:124,58,237;
...
::selection{background:rgba(var(--pri-rgb),.18)}
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;color:var(--txt);font-size:15px;line-height:1.55;padding-bottom:env(safe-area-inset-bottom);
  background:radial-gradient(1100px 480px at 88% -120px,rgba(var(--pri2-rgb),.09),transparent 62%),
             radial-gradient(900px 420px at -8% -60px,rgba(var(--pri-rgb),.09),transparent 58%),
             var(--bg);
...
nav.tabbar .sngroup{flex:none;font-size:11px;font-weight:800;color:var(--sub);letter-spacing:3px;padding:12px 8px 3px;text-align:left}
...
.btn-pri{background:var(--grad);color:#fff;box-shadow:0 2px 10px rgba(var(--pri-rgb),.30)}
...
.chip.active{background:var(--grad);color:#fff;border-color:transparent;box-shadow:0 2px 8px rgba(var(--pri-rgb),.3)}
...
THEME_JS = """
/* ---------- 主题色自定义 ---------- */
var THEME_PRESETS=[['#3b5bfd','#7c3aed','星辰蓝(默认)'],['#0ea5e9','#2563eb','晴空蓝'],['#0a9d6c','#10c98a','松石绿'],['#e58f0b','#f59e0b','琥珀橙'],['#db2777','#f43f5e','玫瑰红'],['#7c3aed','#a855f7','暮光紫'],['#334155','#64748b','石墨灰']];
...
function applyTheme(c1,c2,save){var st=document.documentElement.style,g1=hex2rgb(c1),g2=hex2rgb(c2);
 ...
 st.setProperty('--grad','linear-gradient(135deg,'+c1+','+c2+')');
...
function aiCfg(){
  var s=aiStore();
  return {provider:localStorage.getItem('wb_ai_provider')||'deepseek',
...
function aiReady(){var c=aiCfg();return !!(c.key&&c.base&&c.model)}
...
function ibRawParse(){
  ...
  if(btn){btn.disabled=true;btn.textContent='解析中…'}
  if(tip){tip.style.display='block';tip.textContent='正在解析（'+(aiReady()?'你的模型 Key':'演示模式')+'）…'}
...
<section id="secRs" style="display:none">
  ...
  <p style="margin:0 0 10px;font-size:12px;color:var(--sub)">支持多份简历，每份自己标注求职意向 · txt/md/docx 可直接上传（Word 需较新浏览器），PDF 请复制文本后点「粘贴文本」· 简历内容只存本机浏览器，不上传服务器</p>
  <div class="formrow">
  ...
  <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
    <label class="btn btn-gray" style="cursor:pointer">上传文件（txt/md）<input type="file" id="rsFile" accept=".txt,.md,.markdown,.pdf,.doc,.docx" hidden></label>
...
function aiClearCfg(){
  ...
  $('aiProvider').value='deepseek';$('aiBase').value=AI_PRESETS.deepseek.base;$('aiModel').value=AI_PRESETS.deepseek.model;$('aiKey').value='';$('aiRemember').checked=true;
  aiSetStatus('mock');
}
...
function rsReadFile(f){
  ...
  if(ext==='pdf'||ext==='doc'){alert('浏览器无法直接读取 '+ext.toUpperCase()+'：PDF 请全选复制文本后点「粘贴文本」；老式 .doc 可先另存为 .docx 再上传');return}
  if(ext==='docx'){rsReadDocx(f);return}
  var r=new FileReader();
  r.onload=function(){rsSetText(r.result,f.name);if(!$('rsName').value.trim())$('rsName').value=f.name.replace(/\.[^.]+$/,'')};
...
function rsSaveResume(){
  if(!rsBuf.trim()){alert('请先「上传文件」（txt/md/docx）或「粘贴文本」录入简历内容');return}
...
function rsRender(){
  ...
  if(!a.length){box.appendChild(rsEl('div','empty','还没有简历：上传 txt/md 或粘贴文本，填好求职意向后保存'));return}
