...
<section>
  ...
  <p style="margin:0 0 10px;font-size:12px;color:var(--sub)">支持多份简历，每份自己标注求职意向 · txt/md 可直接上传，PDF/Word 请复制文本后点「粘贴文本」· 简历内容只存本机浏览器，不上传服务器</p>
  <div class="formrow">
...
<div hidden id="tplBox">
  <div class="acard" id="tplApp">
    ...
    <div class="jacts"><a class="btn btn-ghost btn-sm" data-field="相关链接" target="_blank" rel="noopener">链接</a><button type="button" class="btn btn-gray btn-sm aquick">已投</button><button type="button" class="btn btn-gray btn-sm aquick2">不投</button><button class="btn btn-gray btn-sm anote">记笔记</button><button class="btn btn-gray btn-sm adel">删除</button></div>
...
function rsReadFile(f){
  ...
  if(ext==='pdf'||ext==='doc'||ext==='docx'){alert('浏览器无法直接读取 '+ext.toUpperCase()+' 文件：请打开该文件，全选复制文本内容，然后点「粘贴文本」导入（txt / md 文件可直接上传）');return}
...
function rsSaveResume(){
  if(!rsBuf.trim()){alert('请先「上传文件」（txt/md）或「粘贴文本」录入简历内容');return}
