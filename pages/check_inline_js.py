#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""抽取 pages/*.html 与 demo/*.html 里的内联 <script>，交给 `node --check` 做语法检查。

本地：python3 pages/check_inline_js.py
...
def targets():
    """页面清单：4 个生产页 + demo 单文件。"""
    found = sorted(ROOT.glob("pages/*.html")) + sorted(ROOT.glob("demo/*.html"))
...
def main() -> int:
    ...
    files = targets()
    if not files:
        print("FAIL no html pages found under pages/ or demo/")
        return 1
